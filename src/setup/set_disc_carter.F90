...
!--------------------------------------------------------------------------!
module set_disc_carter
!
! This module contains utility routines for accretion disc setups
! Some minor additions have been made to allow a custom random seed
! And a non-mirrored disc
!
! :References:
...
!
! :Original Owner: Daniel Mentiplay
! :Owner of Modified file: Ethan Carter
!
! :Runtime parameters:
!   - G           : *in code units*
!   - M_disc      : *disc mass*
...
subroutine set_disc_positions(npart_tot,npart_start_count,do_mixture,R_ref,R_in,R_out,&
 ...
 #ifdef RANDOM
   write(*,*) 'Provide a (negative integer) seed: '
   read(*,*) iseed
   write(*,*) 'Using user-provided seed: ', iseed
 #else
 ...
 !--loop over particles
 do i=npart_start_count,npart_tot !Remove the 2 so we iterate over all
