...
subroutine get_eos_barotropic_iso(rhoi,polyk_iso,polyk2,ponrhoi,spsoundi,gammai)
 ...
 !    rhocritT0slope = (log10(polyk_iso)-log10(polyk2)) &
 !                   /(log10(rhocritT)-log10(rhocrit0)))
 !

 !For a locally isothermal disc as in Lodato & Pringle (2007),
 !centered on a sink particle.
 !
 !    cs = cs_0*R^(-q) -- polyk is cs^2, so this is (R^2)^(-q)
 !
 !Refer to case (ieos=6); locally isothermal disc for the
 !equation below.
 !
 !This sets a locally isothermal equation of state with radial
 !distance from a sink particle (typically the first, i.e. the star)
 !
 ...
 elseif (rhoi < rhocrit2) then
    gammai  = gamma1
    ponrhoi = polyk_iso*(rhoi/rhocrit1)**(gamma1-1.)
...
!+
!-----------------------------------------------------------------------
real function gamma_barotropic_iso(rhoi) result(gammai)
 real, intent(in) :: rhoi
...
end function gamma_barotropic_iso
...
subroutine eos_info_barotropic_iso(polyk,polyk_iso,polyk2,iprint)
 ...
 write(iprint,"(  2a,2(es10.3,a))")    baro, 'T_iso            = ',T_iso, 'K'
 if (drhocrit0 > 0.0) then
...
subroutine verify_less_than(ierr,val1,val2)
 ...
 if (val1 > val2) then
    ierr = ierr + 1
    call error('eos_barotropic_iso','incorrect ordering of rhocrit')
 endif
